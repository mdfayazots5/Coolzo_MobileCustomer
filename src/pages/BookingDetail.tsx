...
export default function BookingDetail() {
  ...
  useEffect(() => {
    ...
    void BookingService.getBookingById(id).then(setJob).catch(() => setJob(null));
  }, [id]);
  ...
  const status = getStatusConfig(job.operationalStatus || job.status);
        ...
        <div className="flex items-center justify-between mb-12 relative z-10">
          <div className="flex items-center gap-6">
            ...
            <div className="space-y-1">
              ...
              <p className="text-warm-white/30 text-[10px] font-bold uppercase tracking-[0.4em] italic leading-none mt-1">Registry Engagement • {job.serviceRequestNumber || job.bookingReference}</p>
            </div>
          </div>
        ...
        <div className="flex items-center justify-between relative z-10">
          <div className="space-y-3">
             ...
             <div className="flex items-center gap-4">
               ...
               <h2 className="text-[28px] font-display font-bold text-white tracking-tighter leading-none italic uppercase">{job.operationalStatus || job.status}</h2>
             </div>
          </div>
          <div className="text-right space-y-2">
            <p className="text-warm-white/20 text-[10px] font-bold uppercase tracking-[0.5em] italic">Fiscal Index</p>
            <p className="text-[24px] font-display font-bold text-gold tracking-tighter leading-none italic">₹{job.estimatedPrice}</p>
          </div>
        </div>
      ...
      <div className="px-8 -mt-12 space-y-16 relative z-30 pb-40">
        ...
        <section className="bg-white rounded-[60px] p-12 border border-navy/5 shadow-3xl shadow-black/[0.02] space-y-12 relative overflow-hidden group">
          ...
          <div className="space-y-4 relative z-10 italic">
            ...
            <h2 className="text-[36px] font-display font-bold text-navy tracking-tighter leading-none uppercase">{job.serviceName}</h2>
          </div>
          ...
          <div className="grid grid-cols-2 gap-10 pt-4 relative z-10 italic">
            <div className="space-y-3">
              <div className="flex items-center gap-6">
                ...
                <p className="text-[18px] font-bold text-navy tracking-tight uppercase leading-none">{job.slotDate}</p>
              </div>
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-6">
                ...
                <p className="text-[18px] font-bold text-navy tracking-tight uppercase leading-none truncate">{(job.slotLabel || '').split('(')[0]}</p>
              </div>
           ...
           <div className="bg-white rounded-[60px] p-12 border border-navy/5 shadow-3xl shadow-black/[0.02] space-y-12 relative overflow-hidden group">
            ...
            <div className="flex items-start gap-8 group/item">
              ...
              <div className="space-y-3 flex-1 flex items-center justify-between italic">
                ...
                <div className="text-right space-y-2">
                  <p className="text-[36px] font-display font-bold text-navy tracking-tighter leading-none italic">₹{job.estimatedPrice}</p>
        ...
        <section className="space-y-10 pt-10 border-t border-navy/5">
          {(job.operationalStatus || job.status) === 'Completed' ? (
            <Button 
              onClick={() => navigate(`/service-report/${job.bookingId}`)}
            ...
            <div className="flex flex-col gap-6">
              <div className="grid grid-cols-2 gap-6">
                <Button 
                  ...
                  onClick={() => navigate(`/app/reschedule/${job.bookingId}`)}
                >
                  Reschedule
                </Button>
                <Button 
